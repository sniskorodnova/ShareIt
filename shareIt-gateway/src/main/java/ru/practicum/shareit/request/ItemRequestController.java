...
public class ItemRequestController {
    ...
    public Object create(@RequestHeader(value = BaseClient.USER_ID_HEADER, required = false) Long userId,
        ...
        log.info("Входящий запрос на создание запроса на вещь: " + itemRequestCreate.toString());
        if (userId == null) {
    ...
    @GetMapping
    public Object getByRequesterId(@RequestHeader(value = BaseClient.USER_ID_HEADER, required = false) Long userId)
            throws NoHeaderException {
        log.info("Входящий запрос на получение информации по своим запросам для пользователя с id = {}", userId);
        if (userId == null) {
            throw new NoHeaderException("No header in the request");
        } else {
    ...
    public Object getById(@RequestHeader(value = BaseClient.USER_ID_HEADER, required = false) Long userId,
            ...
            throws NoHeaderException {
        log.info("Входящий запрос на получение информации по запросу с id = {}", requestId);
        if (userId == null) {
            throw new NoHeaderException("No header in the request");
        } else {
    ...
    @GetMapping("/all")
    public Object getAll(@RequestHeader(value = BaseClient.USER_ID_HEADER, required = false)
                         Long userId, @RequestParam(required = false, defaultValue = "0") @Min(0) Integer from,
                         @RequestParam(required = false, defaultValue = "10") @Min(1) Integer size)
            throws NoHeaderException {
        log.info("Входящий запрос на получение списка запросов, созданных другими пользователями");
        if (userId == null) {
            throw new NoHeaderException("No header in the request");
        } else {
